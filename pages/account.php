...
$resolveReturnTo = static function (): string {
    $allowed = ['account.php', 'user.php'];
    $target = sanitize((string) ($_POST['return_to'] ?? 'account.php'));
    return in_array($target, $allowed, true) ? $target : 'account.php';
};
    ...
    <div id="page">
        ...
        <main class="container page-shell">
            ...
            <div class="account-layout">
                <section class="card profile-card" style="text-align:center;">
                    <h2>Sua Foto</h2>
                    <form id="avatar-upload-form" method="post" action="account.php?action=avatar"
                        enctype="multipart/form-data">
                        <?= csrfInput(); ?>
                        <input type="hidden" name="return_to" value="account.php">
                    ...
                    <button type="button" id="avatar-trigger" class="avatar-editor" aria-label="Editar foto de perfil"
                        ...
                        <?php if (!empty($user['avatar'])): ?>
                            <img src="<?= e(assetPath((string) $user['avatar'])); ?>"
                                alt="Foto de perfil de <?= e((string) $user['username']); ?>" class="avatar avatar-xl">
                        <?php else: ?>
                            <span
                                class="avatar avatar-xl avatar-fallback"><?= e(usernameInitial((string) $user['username'])); ?></span>
                        <?php endif; ?>
                        <span class="avatar-editor-overlay">Editar foto</span>
                    </button>

                    <?php if (!empty($user['avatar'])): ?>
                        <form id="avatar-remove-form" method="post" action="account.php?action=avatar"
                            style="margin-top:1rem;">
                            <?= csrfInput(); ?>
                            <input type="hidden" name="return_to" value="account.php">
                            <label class="inline-check" for="remove_avatar" style="justify-content:center;">
                ...
                <section class="card form-card">
                    ...
                    <form method="post" action="account.php?action=username">
                        <?= csrfInput(); ?>
                        <label for="username">Username</label>
                        <input type="text" id="username" name="username" value="<?= e((string) $user['username']); ?>"
                            minlength="3" pattern="[A-Za-z0-9_]+" required>
                        <button type="submit" style="margin-top:1rem;">Salvar username</button>
                    </form>
                </section>

                <section class="card form-card full-width-card">
                    <h2>Sobre mim</h2>
                    <form method="post" action="account.php?action=bio">
                        <?= csrfInput(); ?>
                        ...
                        <button type="submit" style="margin-top:1rem;">Salvar bio</button>
                    </form>
                </section>
                ...
                <section class="card form-card full-width-card">
                    ...
                    <form method="post" action="account.php?action=password">
                        ...
                        <button type="submit" style="margin-top:1rem;">Salvar senha</button>
                    </form>
                </section>
            </div>
        </main>
