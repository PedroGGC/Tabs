...
function commentCreate(PDO $pdo, int $currentUserId, array $input): array
{
    ...
    $postStmt = $pdo->prepare('SELECT id, user_id FROM posts WHERE id = :id LIMIT 1');
    ...
    $sendNotification = static function (array $payload) use ($insertNotification): void {
        try {
            $insertNotification->execute($payload);
        } catch (Throwable $e) {
    ...
    if ($parentComment) {
        ...
        if ($parentOwnerId !== $currentUserId && $parentOwnerId !== $postOwnerId) {
            $sendNotification([
...
{
    $isSubdir = str_contains($_SERVER['SCRIPT_NAME'], '/src/actions/') || str_contains($_SERVER['SCRIPT_NAME'], '/pages/');
    $rel = $isSubdir ? '../../' : '';
    // Actually, src/actions is depth 2, pages is depth 1. 
    // Let's use a more precise logic.
    if (str_contains($_SERVER['SCRIPT_NAME'], '/src/actions/')) {
        return '../../pages/post_view.php?id=' . $postId;
    }
    if (str_contains($_SERVER['SCRIPT_NAME'], '/pages/')) {
        return 'post_view.php?id=' . $postId;
    }
    return 'pages/post_view.php?id=' . $postId;
}
